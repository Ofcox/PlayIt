...
std::vector<std::string> LabelMaterial::m_createdLabels;

int LabelMaterial::m_objectCount = 0;

LabelMaterial::LabelMaterial(std::string text ) {
    m_materialName = text;
    m_textureName  = text;
    m_objectCount++;
    ...
    if (!isAlreadyCreated(text)){
        ...
        m_material = MaterialManager::getSingleton().create(m_materialName.c_str(), ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
        ...
        m_material->getTechnique(0)->getPass(0)->createTextureUnitState(m_textureName.c_str() );
    }
}

void LabelMaterial::WriteToTexture( const std::string &str, Image::Box destRectangle, Font *font, const ColourValue &color, char justify, bool wordwrap ) {
